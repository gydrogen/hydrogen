...
std::tuple<std::vector<Path>* , std::vector<Path>*>  Graph::dfs(){
  // std::tuple<std::vector<Path>, std::vector<Path>> rets;
  newlyAdded.clear();
  newlyRemoved.clear();

  for (Graph_Function* func: graphFunctions){
    // perform DFS for every function entry to exit
    Path currentPath(graphVersion);
    std::string fName=func->getFunctionName();
    Graph_Instruction* funcEntry = findVirtualEntry(fName);
    Graph_Instruction* funcExit = findVirtualExit(fName);
    if (funcEntry){
      for (Graph_Edge *edgePtr: funcEntry->getInstructionEdges()){
        Graph_Instruction* toInstr = edgePtr->getEdgeTo();
        currentPath.instructions.push_back(toInstr);
        dfsHelper(&currentPath, funcExit);
      }
    }
  }

  // std::cout << "Printing newly added paths\n";
  // for (Path nap: newlyAdded){
  //   std::cout << "===================================\n";
  //   for (Graph_Instruction * inst: nap.instructions){
  //     std::cout<<inst->getInstructionLabel()+"\n";
  //   }
  //   std::cout << "===================================\n";
  // }

  // std::cout << "Printing newly removed paths\n";
  // for (Path nap: newlyRemoved){
  //   std::cout << "===================================\n";
  //   for (Graph_Instruction * inst: nap.instructions){
  //     std::cout<<inst->getInstructionLabel()+"\n";
  //   }
  ...
  return std::make_tuple(&newlyAdded, &newlyRemoved);
}

void Graph::path_added(){
  // std::tuple<std::vector<Path>, std::vector<Path>> rets = dfs();
  // std::vector<Path> newlyAdded=std::get<0>(rets);
  // std::vector<Path> newlyRemoved=std::get<1>(rets);
  dfs();
  std::cout << "There are "+std::to_string(newlyAdded.size())+" paths added\n";
  std::ofstream aFile("added.txt", std::ios::trunc);
  aFile << "Printing newly added paths\n";
  for (Path nap: newlyAdded){
    aFile << "===================================\n";
    for (Graph_Instruction * inst: nap.instructions){
      aFile <<inst->getInstructionLabel()+"\n";
    }
    aFile << "===================================\n";
  }
  aFile.close();
...
void Graph::path_removed(){
  // std::tuple<std::vector<Path>, std::vector<Path>> rets = dfs();
  // std::vector<Path> newlyAdded=s1td::get<0>(rets);
  // std::vector<Path> newlyRemoved=std::get<1>(rets);

  dfs();
  std::cout << "There are "+std::to_string(newlyRemoved.size())+" paths removed\n";
  std::ofstream rFile("removed.txt", std::ios::trunc);
  rFile <<"Printing newly removed paths\n";
  for (Path nap: newlyRemoved){
    rFile << "===================================\n";
    for (Graph_Instruction * inst: nap.instructions){
      rFile<<inst->getInstructionLabel()+"\n";
    }
    rFile << "===================================\n";
  }
}

void Graph::dfsHelper(Path* currentPath, Graph_Instruction* funcExit){
  Graph_Instruction * currentInst = currentPath->instructions.back();
  // check if this path is newly added or newly removed
  if (currentInst == funcExit){
    bool * allC = currentPath->allContainsBool();
    bool * existsWO = currentPath->existsWithoutBool();
    for (unsigned version=2; version<=graphVersion; version++){
      // if all edges conatins the verison
      if (allC[version-1]){
        if (existsWO[version-2]){
          // then it is newly added
          Path addedPath=*currentPath;
          addedPath.added=true;
          addedPath.arVersion=version;
          newlyAdded.push_back(addedPath);
        }
      }
      // newly removed
      if (allC[version-2]){
        if (existsWO[version-1]){
          // then it is newly removed
          Path removedPath=*currentPath;
          removedPath.removed=false;
          removedPath.arVersion=version;
          newlyRemoved.push_back(removedPath);
        }
      }
    }
  }else{
    for (Graph_Edge *nextEdge : currentInst->getInstructionEdges()){
      if (nextEdge->getEdgeFrom()==currentInst){
        // then this is a to edge, not a from edge
        Graph_Instruction *nextInst = nextEdge->getEdgeTo();
        // std::find(currentPath->instructions.begin(), currentPath->instructions.end(), nextInst);
        // ensure acyclic
        if (std::find(currentPath->instructions.begin(), currentPath->instructions.end(), nextInst)==currentPath->instructions.end()){
          currentPath->instructions.push_back(nextInst);
          for (unsigned edgeVer : nextEdge->getEdgeVersions()){
            currentPath->verCount[edgeVer]++;
          }
          // call recursively
          dfsHelper(currentPath, funcExit);

          // when the call returns, revert the path
          currentPath->instructions.pop_back();
          for (unsigned edgeVer : nextEdge->getEdgeVersions()){
            currentPath->verCount[edgeVer]--;
          }
        }
...
void Graph::printGraph(std::string graphName) {
  ...
  std::cout << "nodes: "+std::to_string(nCount) +"\n";
  std::cout << "edges: "+std::to_string(graphEdges.size()) +"\n";
  ...
  gFile << "\n/* Added Edges */\n";
  for (Path path: newlyAdded){
    bool first=true;
    unsigned fromID;
    unsigned toID;
    for (Graph_Instruction * inst: path.instructions){
      toID = inst->getInstructionID();
      if (!first){
      std::string outputString = "\t\t\"" + std::to_string(fromID) + "\" -> \"" +
                                std::to_string(toID) + "\" [arrowhead = dot, penwidth = 1.0, color = steelblue1, style=dashed];\n";
      gFile << outputString;
      }
      first=false;
      fromID=toID;
  ...
  for (Path path: newlyRemoved){
    ...
    for (Graph_Instruction * inst: path.instructions){
      ...
      if (!first){
      std::string outputString = "\t\t\"" + std::to_string(fromID) + "\" -> \"" +
                                std::to_string(toID) + "\" [arrowhead = odot, penwidth = 1.0, color = violetred1, style=dashed];\n";
      gFile << outputString;
      }
      first=false;
      fromID=toID;
...
std::vector<unsigned> Path::allContains(){
  // return the versions that all edges on this path contains
  std::vector<unsigned> allContainsVersions;
  std::map<unsigned, unsigned>::iterator it = verCount.begin();
  while (it != verCount.end()){
    unsigned version = it->first;
    unsigned count = it->second;
    if (count==this->instructions.size()-1){
      allContainsVersions.push_back(version);
    }
  }
  return allContainsVersions;
}

std::vector<unsigned> Path::existsWithout(){
  // return the versions that some edges on this path do not contain
  std::vector<unsigned> existsWithoutVersions;
  std::map<unsigned, unsigned>::iterator it = verCount.begin();
  while (it != verCount.end()){
    unsigned version = it->first;
    unsigned count = it->second;
    if (count!=this->instructions.size()-1){
      existsWithoutVersions.push_back(version);
    }
  }
  return existsWithoutVersions;
}

bool* Path::allContainsBool(){
